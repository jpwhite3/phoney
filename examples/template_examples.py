...
def make_template_request(template_data, endpoint="/template"):
    ...
    if response.status_code == 200:
        data = response.json()
        print(
            f"✅ Success! Generated {data['generated_count']} records in {duration:.2f}s"
        )
    ...
    template_data = {
        "template": {
            ...
            "author": {
                ...
                "social_media": {"twitter": "{{user_name}}", "website": "{{url}}"},
            },
...
def example_performance_test():
    ...
    if result:
        ...
        print(
            f"Average processing time per record: {result['execution_time_ms'] / result['generated_count']:.3f}ms"
        )
...
def example_validation_test():
    ...
    response = requests.post(
        f"{PHONEY_URL}/api/v1/template/validate", json=invalid_template
    )
...
def example_csv_export():
    ...
    print(
        "Note: CSV export requires authentication. This is a demo of the request format."
    )
...
def run_all_examples():
    ...
    try:
        ...
        print("❌ Could not connect to Phoney server")
        print(
            "Please start the server: poetry run uvicorn phoney.app.main:app --reload"
        )

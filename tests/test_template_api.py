...
class TestSimpleTemplateAPI:
    ...
    def test_simple_template_nested_objects(self, client: TestClient):
        ...
        template_request = {
            "template": {
                ...
                "metadata": {"created": "{{date_time}}", "active": "{{pybool}}"},
            },
            "count": 1,
        }
    ...
    def test_simple_template_validation_errors(self, client: TestClient):
        ...
        template_request = {
            "template": {"name": "{{name}}", "invalid": "{{nonexistent_generator}}"},
            "count": 1,
        }
    ...
    def test_simple_template_empty_template(self, client: TestClient):
        ...
        template_request = {"template": {}, "count": 1}
    ...
    def test_simple_template_malformed_request(self, client: TestClient):
        ...
        response = client.post(
            "/template", json={"template": {"name": "{{name}}"}, "count": 0}
        )
        assert response.status_code == 422
...
class TestAdvancedTemplateAPI:
    ...
    def test_advanced_template_generation(
        ...
        response = client.post(
            "/api/v1/template/generate", json=template_request, headers=auth_headers
        )
    ...
    def test_advanced_template_csv_format(
        self, client: TestClient, auth_headers: dict[str, str]
    ):
    ...
    def test_advanced_template_large_dataset(
        self, client: TestClient, auth_headers: dict[str, str]
    ):
        """Test generation of larger datasets (advanced endpoint only)."""
        template_request = {
            "template": {"id": "{{uuid4}}", "name": "{{name}}", "email": "{{email}}"},
    ...
    def test_advanced_template_unique_values(
        self, client: TestClient, auth_headers: dict[str, str]
    ):
    ...
    def test_advanced_template_without_auth(self, client: TestClient):
        ...
        template_request = {"template": {"name": "{{name}}"}, "count": 1}
...
class TestTemplatePerformance:
    ...
    def test_complex_template_performance(self, client: TestClient):
        ...
        template_request = {
            "template": {
                ...
                "preferences": {
                    "theme": "{{word}}",
                    "notifications": {"email": "{{pybool}}", "sms": "{{pybool}}"},
                },
            },
            "count": 50,
        }
    ...
    def test_validation_performance(self, client: TestClient):
        ...
        assert (
            duration < 2.0
        ), f"Validation took too long: {duration:.3f}s for 50 validations"
...
class TestTemplateErrorHandling:
    ...
    def test_request_validation_errors(self, client: TestClient):
        ...
        invalid_requests = [
            ...
            {"template": {"name": "{{name}}"}, "count": -1},  # Negative count
            {
                "template": {"name": "{{name}}"},
                "count": 10001,
            },  # Count too high (simple endpoint)
        ...
        for invalid_request in invalid_requests:
            ...
            assert response.status_code in [
                400,
                422,
            ], f"Should reject: {invalid_request}"
    ...
    def test_large_request_handling(self, client: TestClient):
        ...
        template_request = {"template": large_template, "count": 10}
    ...
    def test_concurrent_requests(self, client: TestClient):
        ...
        assert (
            success_count >= 8
        ), f"Too many failures in concurrent requests: {status_codes}"
...
class TestTemplateIntegration:
    ...
    def test_full_workflow_advanced_api(
        self, client: TestClient, auth_headers: dict[str, str]
    ):
        ...
        generate_response = client.post(
            "/api/v1/template/generate", json=advanced_request, headers=auth_headers
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "arrays",
            {"employees": "{{[name]:count=5}}", "departments": "{{[word]:count=3}}"},
        ),
        (
            "mixed",
            {
                "name": "{{name}}",
                "friends": "{{[name]:count=2}}",
                "age": "{{random_int:min=25,max=65}}",
            },
        ),
    ],
...
def test_template_types(client: TestClient, template_name: str, template_data: dict):
    ...
    template_request = {"template": template_data, "count": 2, "seed": 42}

...
class TemplateParser:
    ...
    def _parse_string_placeholders(cls, text: str, path: str) -> list[TemplateField]:
        ...
        for match in cls.SIMPLE_PLACEHOLDER.finditer(text):
            ...
            if any(
                field.generator == generator and field.nested_path == path
                for field in fields
            ):
                continue
            ...
            if "." in generator:
                ...
                fields.append(TemplateField(generator=base_generator, nested_path=path))
            else:
                fields.append(TemplateField(generator=generator, nested_path=path))
...
class TemplateValidator:
    ...
    def validate_template(
        self, template: dict[str, Any], strict: bool = False
    ) -> tuple[bool, list[TemplateValidationError], list[str]]:
        """
    ...
    def _validate_field(
        self, field: TemplateField, strict: bool
    ) -> tuple[list[TemplateValidationError], list[str]]:
        ...
        if field.array_count is not None:
            if field.array_count < 1:
                ...
                warnings.append(
                    f"Large array count ({field.array_count}) may impact performance"
                )

        return errors, warnings

    def _validate_parameters(
        self, generator_name: str, parameters: dict[str, Any], field_path: str
    ) -> tuple[list[TemplateValidationError], list[str]]:
        ...
        try:
            ...
            warnings.append(
                f"Could not validate parameters for {generator_name}: {str(e)}"
            )
...
class TemplateProcessor:
    ...
    def process_template(
        self, template: dict[str, Any], count: int = 1, unique: bool = False
    ) -> list[dict[str, Any]]:
    ...
    def _process_template_item(self, item: Any) -> Any:
        ...
        if isinstance(item, dict):
            return {
                key: self._process_template_item(value) for key, value in item.items()
            }
    ...
    def _generate_value(
        self, generator: str, params: dict[str, Any] | None = None
    ) -> Any:
    ...
    def _generate_array(
        self, generator: str, count: int, params: dict[str, Any] | None = None
    ) -> list[Any]:
